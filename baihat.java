...
@WebServlet("/baihat")
public class baihat extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            String searchMBH = req.getParameter("searchMABAIHAT");
...
out.println("</head>");
out.println("<body>");
out.println("<h2 style='color: #fff; background: linear-gradient(to right, #4CAF50, #336699); padding: 15px; border-radius: 8px; border: 2px solid #336699; text-align: center;'>BÀI HÁT</h2>");
...
out.println("<h2>Tìm Kiếm Thông Tin Bài hát </h2>");
out.println("<form method='get'>");
out.println(" <label for='searchMABAIHAT'>Mã bài hát:</label>");
out.println(" <input type='text' id='searchMABAIHAT' name='searchMABAIHAT'>");
...
out.println("<h3>Thêm mới bài hát</h3>");
out.println("<form method='post' action='AddBHServlet'>");
out.println("Mã bài hát: <input type='text' name='MABAIHAT'><br>");
out.println("Tên bài hát: <input type='text' name='TenBaiHat'><br>");
out.println("Giai điệu: <input type='text' name='GiaiDieu'><br>");
out.println("Mã nhạc sĩ: <input type='text' name='MANHACSI'><br>");
out.println("Ghi chú: <input type='text' name='GhiChu'><br>");
...
out.println("</script>");
            ...
            try {
                if (tableName == null || tableName.isEmpty() || tableName.equals("CASI")) {
                    ...
                    String sql1 = "SELECT * FROM BAIHAT";
        String sql2 = "SELECT * FROM BAIHAT WHERE MABAIHAT LIKE '%" + searchMBH + "%'";
                    ResultSet rs;
                     if (searchMBH != null && !searchMBH.isEmpty()) {
            ...
            rs = stmt.executeQuery(sql1);
                    ...
                    out.println("<tr><th>Mã bài hát</th><th>Tên bài hát</th><th>Giai điệu</th><th>mã nhạc sĩ</th><th>Ghi chú</th></tr>");

                    while (rs.next()) {
                        String MBH = rs.getString("MABAIHAT");
                        String TBH = rs.getString("TenBaiHat");
                        String GD = rs.getString("GiaiDieu");
                        String MNS = rs.getString("MANHACSI");
                        String GC = rs.getString("GhiChu");

                        out.println("<tr><td>" + MBH + "</td><td>" + TBH + "</td><td>" + GD + "</td><td>" + MNS
            + "</td><td>" + GC
            + "</td><td><a href='EditBHRecordServlet?MABAIHAT=" + MBH + "'>Edit</a></td>"
            + "<td><a href='DeleteBHRecordServlet?MABAIHAT=" + MBH + "'>Delete</a></td></tr>");
                    }
                }
            } finally {
                if (stmt != null) {
